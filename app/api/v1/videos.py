    ...
    try:
        ...
        for filename in os.listdir(video_dir):
            if filename.lower().endswith(('.mp4', '.webm', '.ogg', '.mov', '.avi')):
                # Check for matching thumbnail (same name with _thumbnail.jpg)
                base = os.path.splitext(filename)[0]
                thumbnail_name = f"{base}_thumbnail.jpg"
                thumbnail_path = video_dir / thumbnail_name
                thumbnail_url = f"/static/videos/{thumbnail_name}" if thumbnail_path.exists() else None
